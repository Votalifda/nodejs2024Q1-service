...
export class FavsService {
  ...
  async findAll() {
      ...
      return {
          artists: allArtists.filter(entity => favArtists.map(item => item.artistId).includes(entity.id)),
          albums: allAlbums.filter(entity => favAlbums.map(item => item.albumId).includes(entity.id)),
          tracks: allTracks.filter(entity => favTracks.map(item => item.trackId).includes(entity.id)),
    };
  }
  async addTrack(id: string) {
      const track = await this.trackRepository.findOne({where: {id}});
      const item = await this.favTracksRepository.findOne({where: {trackId: id}});
      if (!track) {
  ...
  async addAlbum(id: string) {
      ...
      const item = await this.favAlbumsRepository.findOne({where: {albumId: id}});
      if (!album) {
  ...
  async addArtist(id: string) {
      ...
      const item = await this.favArtistsRepository.findOne({where: {artistId: id}});
      if (!artist) {
